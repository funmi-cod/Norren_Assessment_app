...
class SubProductDetailsScreen extends StatelessWidget {
  SubProduct? data;
  SubProductDetailsScreen({this.data, super.key});
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          VerticalSpacing(10.h),
          Styles.medium(
              "${TextLiterals.minFund}:${Utilities.productAmount(currency: data?.currency ?? '', amount: double.tryParse(data?.minFund ?? "0") ?? 0.0)}",
              fontSize: scaledFontSize(12.sp, context),
              color: AppColors.primaryGreen),
          VerticalSpacing(5.h),
          Styles.medium(
              "${TextLiterals.minWithdrawal}:${Utilities.productAmount(currency: data?.currency ?? '', amount: double.tryParse(data?.minWithrawal ?? "0") ?? 0.0)}",
              fontSize: scaledFontSize(12.sp, context),
